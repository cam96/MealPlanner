...
public static class SettingsEndpoints
{
    ...
    private static async Task<Ok<AppSettingsDto>> GetAsync(
        MealPlannerDbContext db,
        CancellationToken cancellationToken)
    {
        var budget = await GetMonthlyBudgetAsync(db, cancellationToken);
        return TypedResults.Ok(new AppSettingsDto(budget));
    }

    private static async Task<Results<Ok<AppSettingsDto>, ValidationProblem>> UpdateAsync(
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (setting is null)
        {
            setting = new AppSetting { Key = MonthlyBudgetKey };
